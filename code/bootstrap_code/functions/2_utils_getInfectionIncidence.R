...
get_infection_incidence_by_deconvolution <- function(
  ...
  for (bootstrap_replicate_i in 0:n_bootstrap) {
    ...
    ### add option 2: smoothing at log-scale 
    # if (smooth_incidence == "smooth-on-log-scale") {
    #   
    #   smoothed_incidence_data_log <- time_series %>%
    #     complete(date = seq.Date(min(date), max(date), by = "days"), fill = list(value = 0)) %>% 
    #     mutate(value = getLOESSCases(dates = date, count_data = ifelse(value>0, log(value), 0)  ))     # using default 21 days for smoothing parameter
    #   
    #   smoothed_incidence_data <- smoothed_incidence_data_log
    #   smoothed_incidence_data$value <- ifelse(smoothed_incidence_data_log$value>0, exp(smoothed_incidence_data_log$value), 0)
    #   
    ...
    if (smooth_incidence == T) {
      smoothed_incidence_data <- time_series %>%
        complete(date = seq.Date(min(date), max(date), by = "days"), fill = list(value = 0)) %>% 
        mutate(value = getLOESSCases(dates = date, count_data = value, days_incl = smooth_para_deConv))
